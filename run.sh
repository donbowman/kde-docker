...
SPICE_LOCAL=${SPICE_LOCAL:-"en_CA.UTF-8"}
TIMEZONE=${TIMEZONE:-"America/Toronto"}
SPICE_USER=${SPICE_USER:-"guest"}
...
echo "$SPICE_USER ALL=NOPASSWD: ALL" > /etc/sudoers.d/$SPICE_USER
...
cd /home/$SPICE_USER
mkdir -p .config
cp /root/kwinrc .config
chown -R $SPICE_USER:$SPICE_USER .config
su $SPICE_USER -c "/usr/bin/Xorg -config /etc/X11/spiceqxl.xorg.conf -logfile  /home/$SPICE_USER/.Xorg.2.log :2 &" 2> /dev/null
su $SPICE_USER -c "DISPLAY=:2 dbus-launch --exit-with-session /usr/bin/startkde"
